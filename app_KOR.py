...
st.sidebar.markdown("# 🍑MCP HOST APP🍑")
...
def get_streaming_callback(text_placeholder, tool_placeholder):
    ...
    def callback_func(message: dict):
        ...
        if isinstance(message_content, AIMessageChunk):
            if isinstance(message_content.content, str):  # ✅ 스트리밍 텍스트 응답일 경우
                accumulated_text.append(message_content.content)
                text_placeholder.markdown("".join(accumulated_text))
            elif isinstance(message_content.content, list):  # tool_use 같은 경우
                message_chunk = message_content.content[0]
                if message_chunk["type"] == "tool_use":
                    ...
                    with tool_placeholder.expander("🛠️ 도구 호출 정보", expanded=True):
